...
class Day2Puzzle2 extends TestCase
{
    public function test()
    {
        for ($i=0; $i < strlen($this->input()->first()); $i++) {
            $find = ($batch = $this->input()
                ->map(function ($line) use ($i) {
                    return substr($line, 0, $i) . substr($line, $i + 1);
                }))
                ->first(function ($line, $key) use ($batch) {
                    $tempBatch = $batch;
                    return $tempBatch->forget($key)->search($line);
                });
       ...
       $this->assertEquals('efmyhuckqldtwjyvisipargno', $find);
    }
