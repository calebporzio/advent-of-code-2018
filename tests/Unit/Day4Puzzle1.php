...
class Day4Puzzle1 extends TestCase
{
    /**
     * I have given up on code quality and creativity for this days challenge.
     * For some reason I've the lost the energy to do a good job on this puzzle (and part 2);
     *
     * @return void
     */
    public function setUp()
    {
        ...
        Collection::macro('mapIntoPattern', function ($expression) {
            return $this->map(function ($string) use ($expression) {
                preg_match($expression, $string, $matches);
                array_shift($matches);
                return $matches;
            });
        });
    ...
    public function test()
    {
        $records = $this->input()
            ->mapIntoPattern('/(\[.*\]) (.*)/')
            ->mapSpread(function($time, $message) {
                return [
            ...
            ->sortBy(function ($record) {
                return $record['time']->getTimestamp();
            })->values();
        ...
        for ($i=0; $i < count($records); $i++) {
            if (preg_match('/Guard #([0-9]*)/', $records[$i]['message'], $matches)) {
                $currentGuardId = $matches[1];

                continue;
            }

            if (isset($shiftsByGuard[$currentGuardId])) {
                $shiftsByGuard[$currentGuardId][] = $records[$i];
            } else {
                $shiftsByGuard[$currentGuardId] = [$records[$i]];
            }
        }

        $guardId = collect($shiftsByGuard)
            ->map(function ($shifts) {
                $minutesAsleep = 0;

                for ($i=0; $i < count($shifts); $i++) {
                    if ($shifts[$i]['message'] === 'wakes up') {
                        $minutesAsleep += $shifts[$i]['time']->diffInMinutes($shifts[$i - 1]['time']);
                    }
                }
            ...
            $shifts = $shiftsByGuard[$guardId];

            for ($i = 0; $i < count($shifts); $i++) {
                if ($shifts[$i]['message'] === 'wakes up') {
                    $listOfMinutesAsleep = array_merge($listOfMinutesAsleep, range($shifts[$i - 1]['time']->minute, $shifts[$i]['time']->minute - 1));
                }
            }

            $mostPopularMinuteAsleep = collect(array_count_values($listOfMinutesAsleep))->sort()->keys()->last();

            $this->assertEquals(151754, $guardId * $mostPopularMinuteAsleep);
    }
